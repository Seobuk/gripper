...
#else
  ssd1306WriteCmd(SSD1306_SEGREMAP & ~0x01); // 역방향
...
bool ssd1306UpdateDraw(void)
{
  ...
  for (i = 0; i < SSD1306_HEIGHT/8; i++)
  {
    // cmd_buffer[0] = 0x00; // Control byte for command
    // cmd_buffer[1] = 0xB0 + i; // Set page address
    // cmd_buffer[2] = 0x00; // Set lower column address
    // i2cWriteData(i2c_ch, i2c_dev, cmd_buffer, 3, 100);

    // cmd_buffer[1] = 0x10; // Set higher column address
    // i2cWriteData(i2c_ch, i2c_dev, cmd_buffer, 3, 100);
    ...
    // DMA를 사용하여 데이터 전송
    uint8_t data_buffer[SSD1306_WIDTH + 1];
    data_buffer[0] = 0x40; // Control byte for data
    memcpy(&data_buffer[1], &ssd1306_buffer[SSD1306_WIDTH * i], SSD1306_WIDTH);    
    
    if (i2cWriteDataDMA(i2c_ch, i2c_dev, data_buffer, SSD1306_WIDTH + 1, 100) == false)
    // if(HAL_I2C_Master_Transmit_DMA(&hi2c1, i2c_dev, data_buffer, SSD1306_WIDTH + 1) != HAL_OK)
    {
      return false;
    }
  }
