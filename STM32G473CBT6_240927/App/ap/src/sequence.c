...
const uint32_t multi_sampling_num     = 5;          // rms 측정시 파형 획득 횟수
...
int32_t  dac_out1 = 0;
int32_t  dac_out2 = 0;
uint16_t freq          = 5;                        // dac freq hz (default 30hz)
...
void sequenceDacUpdate(void)
{
  ...
  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dac_out1); // DAC1 OUT
  HAL_DAC_SetValue(&hdac2, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dac_out2); // DAC2 OUT
