...
// I2C DMA 전송 함수
bool i2cWriteDataDMA(uint8_t ch, uint16_t dev_addr, uint8_t *p_data, uint32_t length, uint32_t timeout)
{
  if (ch >= I2C_MAX_CH)
  {
    return false;
  ...
  lock();
  HAL_StatusTypeDef status = HAL_I2C_Master_Transmit_DMA(i2c_tbl[ch].p_i2c, dev_addr, p_data, length);
  unLock();
  ...
  if (status == HAL_OK)
  {
    ...
    while (HAL_I2C_GetState(i2c_tbl[ch].p_i2c) != HAL_I2C_STATE_READY)
    {
